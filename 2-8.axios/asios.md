...
- all：`axios.all(iterable)`

- spread：`axios.spread(callback)`

#### 创建请求实例

- create：`axios.create(config)`
...
`error`参数中的`reponse`对象和上面的响应结构完全相同，可以用来解析响应体。
...
默认情况下，axios将JavaScript对象序列化为JSON。如果服务器解析数据的方式是表单，我们则要设置请求头并且以application / x-www-form-urlencoded格式发送数据，您可以使用以下选项之一。
