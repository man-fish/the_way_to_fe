...
上面代码中，首先执行 Generator 函数，获取遍历器对象，然后使用`next`方法（第二行），执行异步任务的第一阶段。由于`Fetch`模块返回的是一个 Promise 对象，因此要用`then`方法调用下一个`next`方法，将 `data` 作为参数传入生成器函数，执行后序的操作 `console.log(val)`。

可以看到，虽然 `Generator` 函数将异步操作表示得很简洁，但是流程管理却不方便（即何时执行第一阶段、何时执行第二阶段）。需要在函数外部手动操作。
...
首先我们是可以编写代码让  `Generator` 函数自动执行的。
...
但是，这不适合异步操作。前面的例子我们也看了，异步操作必须保证前一步执行完，还可能会依赖前一步的结果，才能执行后一步。上面的自动执行就不可行。这时，`Thunk` 函数就能派上用处。以读取文件为例。下面的 `Generator` 函数封装了两个异步操作。
...
## co 模块 [§](https://es6.ruanyifeng.com/#docs/generator-async#co-模块) [⇧](https://es6.ruanyifeng.com/#docs/generator-async)
