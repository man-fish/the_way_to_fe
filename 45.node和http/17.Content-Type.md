...
content说明了实体主题内对象的媒体类型，在请求报文中，它用于客户端向服务端说明请求体包含内容的媒体类型。在响应报文中，它说明了服务端向客户端返回内容（响应体）的媒体类型。
...
如果使用AJAX原生POST请求,请求头里设置Content-Type:application/json，multipart/form-data请求的参数会显示在Request Payload中，参数格式为JSON格式：{"key":"value","key":"value"...}，这种方式可读性会更好。
...
2. 原生的AJAX请求头里设置Content-Type:application/json，multipart/form-data（文件）或者使用默认的请求头Content-Type:text/plain;参数会显示在Request payload块里提交。
...
在响应报文中，Content-Type一般包含着响应体的MIME类型。
