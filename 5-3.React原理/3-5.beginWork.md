...
<img src="assets/fiberDiff (2).png" alt="fiberDiff (2)" style="zoom:80%;" />

第二轮，虽然两个节点乱序了，但是新节点是一个文本节点而且老节点上没有 `key`，所以不会跳出循环，这种情况会新建一个 `textNode` 并且标记 `Placement`，之后 `oldFiber` 指针在下一轮不会移动：

<img src="assets/fiberDiff2.png" alt="fiberDiff2" style="zoom:80%;" />

在下一次节点比较的时候发现 `newChild` 和 `oldFiber` 的 `key` 不相同，所以跳出了第一个循环：

<img src="assets/fiberDiff3.png" alt="fiberDiff3" style="zoom:80%;" />
...
第一步先根据 `oldFiber` 的 `index` 和 `key` 做一个节点映射，之后从 `newChild` 做匹配，先匹配到了 `tag:g` 但是由于没有复用，所以设置 `placement` 标记位。

![fiberDiff4 (1)](assets/fiberDiff4 (1).png)

最后到了 `tag:x`，匹配成功而且可以重建，并且 `lastPlacedIdx(0) < newIdx(3)`所以不需要设置 `placement`标志位。

![fiberDiff5](assets/fiberDiff5.png)
