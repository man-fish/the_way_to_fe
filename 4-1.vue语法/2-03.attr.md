...
一个非 `prop` 的 `attribute` 是指传向一个组件，但是该组件并没有相应 [props](https://vue3js.cn/docs/zh/guide/component-props) 或 [emits](https://vue3js.cn/docs/zh/guide/component-custom-events.html#defining-custom-events) 定义的 `attribute`。常见的示例包括 `class`、`style` 和 `id` 等`dom`属性。
...
与单根节点组件不同，有多个根节点的组件不具有自动分配 `attribute` 行为。如果未显式绑定 `$attrs`，将发出运行时警告。
